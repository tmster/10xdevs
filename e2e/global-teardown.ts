...
async function globalTeardown() {
  ...
  try {
    ...
    const { data: generations } = await supabase.from("generations").select("id").eq("user_id", testUserId);

    if (generations && generations.length > 0) {
      const generationIds = generations.map((gen) => gen.id);

      // Delete error logs associated with these generations
      await supabase.from("generation_error_logs").delete().in("generation_id", generationIds);

      // 2. Delete flashcards associated with these generations
      await supabase.from("flashcards").delete().in("generation_id", generationIds);
    }
