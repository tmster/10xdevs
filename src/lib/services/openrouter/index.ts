import type { OpenRouterConfig, ChatCompletionParams, ChatCompletionResponse, RequestBody, OpenRouterAPIResponse, ChatMessage, ResponseFormat } from './types';
...
export class OpenRouterService {
  ...
  private validateMessages(messages: ChatMessage[]): void {
    ...
    for (const message of messages) {
      ...
      if (!message.content || typeof message.content !== 'string' || message.content.trim().length === 0) {
        throw new OpenRouterError(
          'Invalid request: each message must have non-empty content',
          'INVALID_REQUEST'
        );
      }
    }
  ...
  private validateResponseFormat(format: ResponseFormat | undefined): void {
    ...
    if (format.type !== 'json_object') {
      throw new OpenRouterError(
        'Invalid request: response format type must be json_object',
        'INVALID_REQUEST'
      );
    }
    ...
    if (!format.json_schema || typeof format.json_schema !== 'object') {
      throw new OpenRouterError(
        'Invalid request: response format must have a json_schema object',
        'INVALID_REQUEST'
      );
    }
    ...
    if (format.json_schema.type !== 'object') {
      throw new OpenRouterError(
        'Invalid request: json_schema type must be object',
        'INVALID_REQUEST'
      );
    }
    ...
    if (!format.json_schema.properties || typeof format.json_schema.properties !== 'object') {
      throw new OpenRouterError(
        'Invalid request: json_schema must have properties object',
        'INVALID_REQUEST'
      );
    }
  }
  ...
  private validateParameters(params: ChatCompletionParams): void {
    ...
    if (params.temperature !== undefined && (
      ...
      throw new OpenRouterError(
        'Invalid request: temperature must be a number between 0 and 2',
        'INVALID_REQUEST'
      );
    }
    ...
    if (params.maxTokens !== undefined && (
      ...
      throw new OpenRouterError(
        'Invalid request: maxTokens must be a positive number',
        'INVALID_REQUEST'
      );
    }
  }
  ...
  private formatRequest(params: ChatCompletionParams): RequestBody {
    ...
    return {
      messages: [
        { role: 'system', content: params.systemMessage || this.defaultSystemMessage },
        ...params.messages
      ],
